...
void ListAllCampaignsForCustomer(vector<Customer> &AllCustomers, int indexForCustomer)
{
	...
	cout << endl << " --- All Campaigns for customer " << AllCustomers[indexForCustomer].GetName() << " --- " << endl;
...
void AdvertisementMenu(vector<Customer> &AllCustomers, int indexForCustomer, IdGenerator &idGenerator)
{
	...
	cout << endl << " -> You are here: [Main menu > Edit Customer > Advertisement menu]" << endl << endl;
	...
	while (true)
	{
		...
		case 3:
			cout << " -> Enter campaign ID for the advertisement: " << endl << " -> "; cin >> campaignIdBuf;
			cout << " -> Enter ID for the advertisement to delete: " << endl; cout << " -> "; cin >> advertisementId;
...
void test_main()
{
	...
	the_client.CommitAdvertisement(Ad("SALE!!", "theClient", 123456, AdType::SCROLL), 1234);
	the_client.CommitAdvertisement(Ad("SuperSale", "theClient", 123456789, AdType::SCROLL), 1234);

	the_client.CommitAdvertisement(Ad("New year craze, must buy!", "Soweeeeet", 123456, AdType::BLINK), 12345);
	the_client.CommitAdvertisement(Ad("Are you feeling hungry?", "Soweeeeet", 123456789, AdType::BLINK), 12345);
	...
	the_other_client.CommitAdvertisement(Ad("SALE!!", "theOtherClient", 234565, AdType::SCROLL), 456);
	the_other_client.CommitAdvertisement(Ad("SuperDUPERSale", "theOtherClient", 35377, AdType::SCROLL), 456);

	the_other_client.CommitAdvertisement(Ad("New year never felt so good!", "theOtherClient", 3456345, AdType::BLINK), 789);
	the_other_client.CommitAdvertisement(Ad("Are you feeling hungry?", "theOtherClient", 5677467, AdType::BLINK), 789);
